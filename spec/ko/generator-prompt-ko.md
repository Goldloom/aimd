...
소스의 모든 사실은 canonical 출력에 반드시 존재해야 합니다. 의미를 생략, 요약, 재해석하지 마십시오.
...
NOTE — ACP와 변환의 구분 (흔한 오해 주의):
"투영(Projection)"은 오직 ACP에만 해당합니다: 완성된 canonical 문서에서 역할별(FE/BE/QA) 라인을 선택하는 작업입니다.
ACP 투영은 변환 이후의 하위 작업이며, MD→AIMD 변환 방식과 무관합니다.
변환 자체는 투영이 아닙니다 — 의미를 보존하는 구조적 재구성(lossless restructuring)입니다.
12. ACP(역할별 뷰)는 canonical 라인을 선택(projection)하여 도출해야 합니다 — 재작성(rewriting)이 아닙니다.
```
...
인간용 서술 문장이 존재할 경우:
- 이를 라인 단위로 그대로 미러링하지 마십시오.
- 압축되고 정규화된 정형 페이로드로 변환하십시오.
- 형식(form)을 압축하되, 의미(meaning)는 반드시 보존하십시오. 모든 사실이 출력에 존재해야 합니다.
...
### 시나리오별 대응 지침:
- [CASE: CONVERT] 입력이 마크다운이나 일반 서술형일 경우: 의도를 정형화된 라인 메모리로 정규화하십시오. (의미 보존 필수)
