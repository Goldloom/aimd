...
- 압축 대상: 표현 형식 (장황함, 중복, 서술적 노이즈)
- 압축 금지 대상: 의미 내용 (사실, 제약, 상태)
- 소스에 존재하는 모든 사실은 반드시 canonical 라인으로 출력에 존재해야 합니다

이 원칙은 ACP와 별개입니다. ACP 투영(완성된 canonical 문서에서 역할별 라인 선택)은 하위 작업이며, MD→AIMD 변환의 무손실 요구사항에 영향을 주지 않습니다.
...
### 9.2 마크다운 → AIMD: 헤더/리스트/표를 정형 사실로 보존 및 수치/기한 추출에 집중. 소스의 모든 사실은 반드시 canonical 출력에 존재해야 합니다(무손실 변환). 형식만 압축하고 의미를 생략·요약·재해석하지 마십시오.
...
6. MD→AIMD 변환을 의도적 손실 압축으로 취급하는 행위 — 형식 정규화 대신 소스 사실을 생략하거나 요약하는 것
