...
[![AIMD Spec](https://img.shields.io/badge/AIMD-v1.5--Canonical-orange)](spec/ko/AIMD-v1.5-ko.md)
...
*   **[AI 아키텍트 실사용 후기 (docs/experience/)](docs/experience/260331_AI_ARCHITECT_LOG.md)**: AIMD v1.5를 이용한 실제 SaaS 구축기 및 정합성 검증 결과.

---

## 💡 실제 사례 (Case Study)

AIMD v1.5는 단순한 이론이 아닙니다. 실제 엔터프라이즈급 SaaS 프로젝트(`aimd-cloud`)를 구축하며 그 파괴력이 검증되었습니다. AI 에이전트가 직접 작성한 실감 나는 후기를 확인해 보세요.

*   **[AI 아키텍트의 AIMD v1.5 실사용 후기 (한글)](docs/experience/260331_AI_ARCHITECT_LOG.md)**
...
| `verifiable references` | 라인 간 크로스 레퍼런스는 반드시 검증기에 의해 체크 가능해야 함 |
...
| `:::state` | 확인된 사실/마일스톤, 미결 이슈, 가설, 다음 액션 | `v` (+ `@date`), `o`, `a`, `n`, `ask` |
| `:::flow` | 실행 단계 | `s` (+ `ref()`) |
| `:::test` | 선언적 검증 기준 | `t` |

선택적 블록: `:::schema`, `:::api`, `:::ref`, `:::human`, `:::diff`
...
- `ref(<id>...)`: 라인 간 의존성 명시 (§10.4)
- `@YYYY-MM-DD`: `v` 라인을 위한 비의미적 메타데이터 (§10.5)
- 짧고 정규화된 형태 유지
- 코어 블록 내 서술형 문장 금지
- 라인당 하나의 사실만 기재
...
*   [`spec/en/`](spec/en/): 영문 핵심 사양 및 가이드라인
    *   [`AIMD-v1.5.md`](spec/en/AIMD-v1.5.md): AIMD 코어 문법 및 블록 규격.
    *   [`generator-spec.md`](spec/en/generator-spec.md): AI 모델 생성기 엔진 사양.
*   [`spec/ko/`](spec/ko/): 한글 핵심 사양 및 가이드라인
    *   [`AIMD-v1.5-ko.md`](spec/ko/AIMD-v1.5-ko.md): AIMD 코어 문법 및 블록 규격.
...
현재 버전: **v1.5** (Proposed Spec)

AIMD v1.5는 제안된 사양 단계입니다. 구현 및 피드백을 언제든지 환영합니다.

---

## 💻 AI 워크스페이스 설정 가이드 (AI-Native Setup Guide)

이 프로젝트의 AIMD v1.5 규격을 자신의 실무 프로젝트에 즉시 적용하려면 다음 인프라 파일들을 자신의 프로젝트 루트(Root)로 복사하십시오:

### 1단계: 전역 규칙 활성화 (`.cursorrules`)
프로젝트 루트에 `.cursorrules` 파일을 배치하면, Cursor나 Copilot 같은 AI가 자동으로 AIMD v1.5 규격을 준수합니다.
