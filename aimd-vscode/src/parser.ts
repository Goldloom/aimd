...
export function parseAbbr(blocks: AimdBlock[]): Record<string, string> {
...
export function buildForAi(filePath: string): string {
  ...
  const shared = blocks.filter(block => !AI_ONLY_BLOCKS.has(block.type) && !HUMAN_ONLY_BLOCKS.has(block.type));
  ...
  if (mergedAi.length > 0) {
    ...
    for (const block of mergedAi) parts.push(`:::ai ${block.attrs}\n${block.content}\n:::`);
  }
  for (const block of mergedRules) parts.push(`:::rules ${block.attrs}\n${block.content}\n:::`);
  for (const block of mergedAbbr) parts.push(`:::abbr\n${block.content}\n:::`);
  for (const block of shared) parts.push(`:::${block.type} ${block.attrs}\n${block.content}\n:::`);
...
export function buildForAcp(filePath: string, role: AcpRole = 'general'): string {
  ...
  const projectName = frontMatter.project?.toString() || path.basename(filePath, '.aimd');
  ...
  const shared = blocks.filter(block => roleSet.has(block.type));
  const parts: string[] = [':::summary', (firstParagraph(prose) || `${projectName} ${role} ACP`).trim(), ':::'];

  for (const block of mergedAi) parts.push(`:::ai role:${role}\n${block.content}\n:::`);
  for (const block of mergedRules) if (roleSet.has('rules')) parts.push(`:::rules ${block.attrs}\n${block.content}\n:::`);
  for (const block of mergedAbbr) if (roleSet.has('abbr')) parts.push(`:::abbr\n${block.content}\n:::`);
  for (const block of shared) {
    if (block.type === 'rules' || block.type === 'abbr' || block.type === 'human' || block.type === 'ai') continue;
...
export function normalizeAimdContent(raw: string, filename: string): string {
  ...
  if (!existingTypes.has('schema')) {
    for (const candidate of analysis.schemaCandidates) prefixes.push(renderSchemaBlock(candidate));
  }
...
function analyzeSource(sourceText: string, fallbackTitle: string): GenerationAnalysis {
  ...
  if (apiEntries.length > 0) verifiedItems.push(`Extracted ${apiEntries.length} explicit API pattern(s) from source text.`);
  ...
  if (ruleItems.length > 0) verifiedItems.push(`Normalized ${ruleItems.length} rule or constraint statement(s) from source prose.`);
...
function renderGeneratedAimd(
  ...
  for (const candidate of analysis.schemaCandidates) parts.push(renderSchemaBlock(candidate));
...
function renderIntentBlock(analysis: GenerationAnalysis): string {
  ...
  if (analysis.domainHints.length > 0) lines.push(`domains: ${analysis.domainHints.join(', ')}`);
  lines.push(':::');
  return lines.join('\n');
}
...
function renderFlowBlock(analysis: GenerationAnalysis): string {
  ...
  analysis.flowSteps.forEach((step, index) => lines.push(`${index + 1}. ${step}`));
  lines.push(':::');
  return lines.join('\n');
}
...
function inferFlowTemplate(domainHints: string[], apiEntries: string[], schemaCandidates: SchemaCandidate[]): string[] {
...
function inferTestItems(domainHints: string[], flowSteps: string[], apiEntries: string[], criticalItems: string[]): string[] {
  const tests: string[] = [];
  ...
  if (domainHints.includes('auth-login')) tests.push('invalid credentials -> authentication rejected');
  if (domainHints.includes('password-reset')) tests.push('expired or invalid reset challenge -> rejected');
  if (flowSteps.length > 0) tests.push('main flow step sequence -> completes without skipped transitions');
...
function tableRowToSchemaLine(headers: string[], row: string[]): string | null {
  ...
  const field = firstNonEmpty(pickValue(values, /(field|name|column|property|key|필드|컬럼)/), row[0]) ?? '';
  if (!field) return null;

  const typeRaw = firstNonEmpty(pickValue(values, /(type|datatype|format|타입|자료형)/), row[1]) ?? 'string';
  ...
  const defaultValue = normalizeDefaultValue(defaultRaw);
...
function normalizeDefaultValue(raw: string): string {
